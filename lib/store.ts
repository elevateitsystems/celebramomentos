...
import { defaultEmojiPosition, type EmojiPosition, type EmojiTone } from "./emojis";

export type CartState = {
  card: { templateId: string; frameId: string | null; message: string; emoji: string; emojiTone: EmojiTone; emojiSize: number; emojiPosition: EmojiPosition; photoDataUrl: string | null; photoZoom: number; photoPosition: "center" | "top" | "bottom" | "left" | "right"; size: CardSize; delivery: DeliveryMethod; recipientMode: RecipientMode };
...
const initialState: CartState = {
  card: { templateId: "template-1", frameId: "floral", message: "Feliz vuelta al sol, Ana ✨", emoji: "✨", emojiTone: "natural", emojiSize: 28, emojiPosition: defaultEmojiPosition, photoDataUrl: null, photoZoom: 1, photoPosition: "center", size: "A4", delivery: "standard", recipientMode: "recipient" },
  stickerQuantity: 0,
  stickerImage: null,
  chocolates: {},
  discountCode: "",
...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    setEmojiTone: (state, action: PayloadAction<EmojiTone>) => { state.card.emojiTone = action.payload; },
    setEmojiSize: (state, action: PayloadAction<number>) => { state.card.emojiSize = Math.min(64, Math.max(18, action.payload)); },
    setEmojiPosition: (state, action: PayloadAction<EmojiPosition>) => { state.card.emojiPosition = action.payload; },
...
export const { setTemplate, setFrame, setMessage, setEmoji, setEmojiTone, setEmojiSize, setEmojiPosition, setPhoto, setPhotoZoom, setPhotoPosition, setSize, setDelivery, setRecipientMode, setStickerQuantity, setStickerImage, setChocolateQuantity, setDiscountCode, resetCart } = cartSlice.actions;
