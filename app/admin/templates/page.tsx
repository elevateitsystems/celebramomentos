...
export default function AdminTemplatesPage() {
  ...
  return <AdminShell><AdminPageHeader eyebrow="CATALOG / DESIGN SYSTEM" title="Templates" body="Create and edit the starting points customers use to make their card their own." action={<button onClick={startNew} className="rounded-full bg-[#ee5264] px-4 py-3 text-xs font-black text-white">+ New template</button>} />
    <div className="mt-8 grid gap-5 md:grid-cols-3">{items.map((template) => <article key={template.id} className="overflow-hidden rounded-2xl border border-[#e4e7ee] bg-white"><div className="relative flex aspect-[1.25/1] items-center justify-center bg-[#f8efe8] p-4"><div className="aspect-[.82/1] h-full overflow-hidden rounded-lg shadow">{template.layoutId === "template-magazine" ? <CardPreview compact templateId="template-magazine" message="Feliz cumpleaños" emojiElements={[]} photoDataUrls={[null, null, null, null]} /> : <ShowcaseCard variant={template.layoutId === "template-3" ? "graduation" : "romantic"} language="es" />}</div><span className="absolute left-4 top-4 rounded-full bg-white px-3 py-1.5 text-[10px] font-black tracking-wider">{template.eyebrow}</span></div><div className="p-5"><h2 className="font-black">{template.name}</h2><p className="mt-2 min-h-12 text-sm leading-6 text-[#737b90]">{template.description}</p><div className="mt-5 flex items-center justify-between"><span className="text-sm font-black">€{template.price.toFixed(2)}</span><button onClick={() => startEdit(template)} className="flex items-center gap-2 text-xs font-black text-[#ee5264]">Edit <Icon name="edit" size={14} /></button></div></div></article>)}</div>
