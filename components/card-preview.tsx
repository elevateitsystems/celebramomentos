...
export function CardPreview({ templateId, message, emoji = "✨", emojiTone, emojiSize, emojiPosition, emojiElements, onEmojiPositionChange, onEmojiElementPositionChange, onEmojiSelect, photoDataUrl, photoDataUrls, photoZoom = 1, photoPosition = "center", frameId, size = "A4", compact = false, className = "", photoFit = "cover" }: CardPreviewProps) {
  ...
  return <div ref={designRef} className={`relative overflow-hidden rounded-[12px] bg-[#fff0e8] ${surfaceClass} ${className}`}>
    {template.magazineStyle ? <MagazineFront compact={compact} message={message} photoDataUrl={photos[0]} photoZoom={photoZoom} photoPosition={photoPosition} /> : template.id === "template-2" ? <CollageFront compact={compact} message={message} photos={photos} frameImage={frame.image} photoZoom={photoZoom} photoPosition={photoPosition} photoFit={photoFit} /> : <StoryFront compact={compact} message={message} photos={photos} frameImage={frame.image} photoZoom={photoZoom} photoPosition={photoPosition} photoFit={photoFit} />}
...
function PhotoTile({ src, fallback, alt, className, photoZoom = 1, photoPosition = "center", photoFit = "cover" }: { src: string | null; fallback: string; alt: string; className: string; photoZoom?: number; photoPosition?: string; photoFit?: "cover" | "contain" | "fill" }) {
  const fitClass = photoFit === "contain" ? "object-contain" : photoFit === "fill" ? "object-fill" : "object-cover";
  return <div className={`relative overflow-hidden bg-[#eee5df] ${className}`}><img src={src || fallback} alt={alt} className={`h-full w-full ${fitClass} ${src ? "" : "opacity-75 saturate-[.8]"}`} style={{ objectPosition: photoPosition, transform: src && photoFit === "cover" ? `scale(${photoZoom})` : undefined }} />{!src && <span className="absolute inset-x-0 bottom-0 bg-[#182443]/70 px-2 py-1 text-center text-[clamp(5px,1vw,9px)] font-black uppercase tracking-wider text-white">Añade foto</span>}</div>;
}

function CollageFront({ compact, message, photos, frameImage, photoZoom, photoPosition, photoFit }: { compact: boolean; message: string; photos: (string | null)[]; frameImage: string; photoZoom: number; photoPosition: string; photoFit: "cover" | "contain" | "fill" }) {
  return <div className="absolute inset-0"><img src={frameImage} alt="" className="absolute inset-0 h-full w-full object-fill opacity-65" /><div className="absolute inset-[8%] flex flex-col rounded-lg border border-white/90 bg-white/90 p-[5%] shadow-lg"><div className="mb-[4%] border-l-4 border-[#ee5264] pl-[4%]"><span className={`font-black uppercase tracking-[.14em] text-[#ee5264] ${compact ? "text-[4px]" : "text-[clamp(7px,1.2vw,11px)]"}`}>Tu colección de momentos</span><p className={`mt-[2%] line-clamp-2 font-serif font-bold leading-tight text-[#182443] ${compact ? "text-[9px]" : "text-[clamp(19px,3.2vw,32px)]"}`}>{message || "Enhorabuena"}</p></div><PhotoTile src={photos[0]} fallback={samplePhotos[0]} alt="Foto 1 · Portada" className="min-h-0 flex-1 rounded-md" photoZoom={photoZoom} photoPosition={photoPosition} photoFit={photoFit} /></div></div>;
}

function StoryFront({ compact, message, photos, frameImage, photoZoom, photoPosition, photoFit }: { compact: boolean; message: string; photos: (string | null)[]; frameImage: string; photoZoom: number; photoPosition: string; photoFit: "cover" | "contain" | "fill" }) {
  return <div className="absolute inset-0">
    <img src={frameImage} alt="" className="absolute inset-0 h-full w-full object-fill opacity-65" />
    <div className={`absolute flex flex-col items-center rounded-[clamp(8px,1.4vw,18px)] border border-white/90 bg-white/90 shadow-lg ${compact ? "inset-[8%] p-[5%]" : "inset-[7%] p-[5.5%]"}`}>
      <PhotoTile
        src={photos[0]}
      ...
      <div className="mt-[6%] flex min-h-0 w-[92%] flex-1 flex-col items-center text-center">
        <span className={`font-black uppercase tracking-[.14em] text-[#ee5264] ${compact ? "text-[4px]" : "text-[clamp(7px,1.2vw,11px)]"}`}>Una tarjeta solo tuya</span>
        <p className={`mt-[3%] line-clamp-2 whitespace-pre-line break-words font-serif font-bold leading-[1.02] text-[#182443] ${compact ? "text-[10px]" : "text-[clamp(22px,4vw,40px)]"}`}>{message || "Enhorabuena"}</p>
      </div>
    </div>
  </div>;
}
