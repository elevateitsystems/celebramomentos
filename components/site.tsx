...
export function SiteHeader({ active = "" }: { active?: string }) {
  ...
  const links = [[t.home, "/"], [t.designs, "/templates"], [t.how, "/#how"], [t.extras, "/addons"], [t.help, "/contact"]];
  return <>
    ...
    <header className="sticky top-0 z-30 border-b border-[#ebdfd6]/80 bg-[#fffaf5]/90 backdrop-blur-md">
      <div className="container flex h-[76px] items-center justify-between gap-4">
        <Link href="/" className="flex items-center gap-3"><img src="/images/logo.png" alt="Celebra Momentos" className="h-12 w-12 object-contain" /><span className="hidden text-[15px] font-black tracking-[-.03em] sm:block">Celebra<br /><span className="text-[#ee5264]">Momentos</span></span></Link>
        ...
        <div className="flex items-center gap-2"><div className="hidden rounded-full border border-[#dfd3cc] bg-white p-1 text-[11px] font-black sm:flex"><button onClick={() => dispatch(setLanguage("es"))} className={`rounded-full px-3 py-1.5 ${language === "es" ? "bg-[#182443] text-white" : "text-[#7b8193]"}`}>ES</button><button onClick={() => dispatch(setLanguage("pt"))} className={`rounded-full px-3 py-1.5 ${language === "pt" ? "bg-[#182443] text-white" : "text-[#7b8193]"}`}>PT</button></div><Link href="/account" className="focus-ring hidden h-10 items-center gap-2 rounded-full border border-[#dfd3cc] bg-white px-3 text-[12px] font-bold text-[#182443] transition hover:border-[#ee5264] md:flex"><Icon name="user" size={17} />{t.account}</Link><Link href="/cart" className="focus-ring flex h-10 items-center gap-2 rounded-full bg-[#ee5264] px-3 text-[12px] font-bold text-white transition hover:bg-[#d83d54]"><Icon name="bag" size={17} /><span className="hidden md:inline">{t.selection}</span></Link><button onClick={() => setMobileMenu(!mobileMenu)} className="focus-ring flex h-10 w-10 items-center justify-center rounded-full border border-[#dfd3cc] bg-white lg:hidden" aria-label="Abrir menú"><Icon name="menu" size={18} /></button></div>
      </div>
      {mobileMenu && <div className="border-t border-[#ebdfd6] bg-white px-5 py-4 lg:hidden"><nav className="container flex flex-col gap-4 text-sm font-bold text-[#59627b]">{links.map(([label, href]) => <Link key={label} href={href} onClick={() => setMobileMenu(false)}>{label}</Link>)}<Link href="/account" onClick={() => setMobileMenu(false)}>{t.account}</Link><div className="flex gap-2 pt-1"><button onClick={() => dispatch(setLanguage("es"))} className={`rounded-full border px-3 py-1 text-xs ${language === "es" ? "border-[#182443] bg-[#182443] text-white" : "border-[#dfd3cc]"}`}>Español</button><button onClick={() => dispatch(setLanguage("pt"))} className={`rounded-full border px-3 py-1 text-xs ${language === "pt" ? "border-[#182443] bg-[#182443] text-white" : "border-[#dfd3cc]"}`}>Português</button></div></nav></div>}
    </header>
  </>;
}

export function SiteFooter() {
  return <footer className="border-t border-[#ebdfd6] bg-white py-12"><div className="container grid gap-10 md:grid-cols-[1.5fr_1fr_1fr] md:gap-20"><div><div className="flex items-center gap-3"><img src="/images/logo.png" alt="Celebra Momentos" className="h-12 w-12 object-contain" /><span className="text-[15px] font-black tracking-[-.03em]">Celebra<br /><span className="text-[#ee5264]">Momentos</span></span></div><p className="mt-5 max-w-[300px] text-sm leading-6 text-[#737b90]">Tarjetas que convierten un día cualquiera en un momento para recordar.</p></div><div><p className="text-xs font-black uppercase tracking-[.15em] text-[#182443]">Explora</p><div className="mt-5 flex flex-col gap-3 text-sm text-[#737b90]"><Link href="/templates" className="hover:text-[#ee5264]">Diseños</Link><Link href="/#how" className="hover:text-[#ee5264]">Cómo funciona</Link><Link href="/addons" className="hover:text-[#ee5264]">Extras</Link></div></div><div><p className="text-xs font-black uppercase tracking-[.15em] text-[#182443]">Información</p><div className="mt-5 flex flex-col gap-3 text-sm text-[#737b90]"><Link href="/privacy" className="hover:text-[#ee5264]">Privacidad</Link><Link href="/cookies" className="hover:text-[#ee5264]">Cookies</Link><Link href="/terms" className="hover:text-[#ee5264]">Términos</Link><Link href="/contact" className="hover:text-[#ee5264]">Contacto</Link></div></div></div><div className="container mt-10 flex flex-col gap-3 border-t border-[#ebdfd6] pt-5 text-xs text-[#9297a4] sm:flex-row sm:items-center sm:justify-between"><span>© 2026 Celebra Momentos</span><span className="flex items-center gap-2"><Icon name="lock" size={13} /> Compra sencilla · Atención humana</span></div></footer>;
}
